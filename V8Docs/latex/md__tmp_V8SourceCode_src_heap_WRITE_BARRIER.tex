...
The generational barrier is always enabled, while the other barriers are only enabled while incremental/concurrent marking is running.\hypertarget{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md20}{}\doxysection{Overview}\label{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md20}
...
CSA and the interpreter do not have their own barrier but reuse Turbofan\textquotesingle{}s write barrier. While implementations for compilers differ, the general idea is still pretty much the same for all of them.\hypertarget{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md21}{}\doxysubsection{Fast path  (\#1)}\label{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md21}
...
\end{DoxyItemize}\hypertarget{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md22}{}\doxysubsection{Deferred code path (\#2)}\label{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md22}
...
\end{DoxyItemize}\hypertarget{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md23}{}\doxysubsection{Slow path (\#3)}\label{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md23}
The slow path of the barrier is shared between all compilers and call sites. In the generational barrier the field/slot is simply recorded in the old-\/to-\/new slot set. During incremental/concurrent marking it also marks the {\ttfamily value} object and may record the slot in the old-\/to-\/old remembered set for pointers to evacuation candidates. This part of the barrier is implemented in CSA in \href{https://source.chromium.org/chromium/chromium/src/+/main:v8/src/builtins/builtins-internal-gen.cc?q=WriteBarrierCodeStubAssembler}{\texttt{ builtins-\/internal-\/gen.\+cc}} and used in the \href{https://source.chromium.org/chromium/chromium/src/+/main:v8/src/builtins/builtins-internal-gen.cc?q=RecordWriteSaveFp}{\texttt{ Record\+Write\+XXX}} builtins.\hypertarget{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md24}{}\doxysubsection{C++ slow path (\#4)}\label{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md24}
Inserting into the slot set in the \href{https://source.chromium.org/chromium/chromium/src/+/main:v8/src/builtins/builtins-internal-gen.cc?q=RecordWriteSaveFp}{\texttt{ Record\+Write\+XXX}} builtins may need to {\ttfamily malloc()} memory. The builtin therefore may call into C++ for this operation. The C++ function for this is \href{https://source.chromium.org/chromium/chromium/src/+/main:v8/src/heap/heap.cc?q=Heap::InsertIntoRememberedSetFromCode}{\texttt{ Heap\+::\+Insert\+Into\+Remembered\+Set\+From\+Code}}.\hypertarget{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md25}{}\doxysection{Indirect pointer barrier}\label{md__tmp_V8SourceCode_src_heap_WRITE_BARRIER_autotoc_md25}
